...
def load_data():
    ...
    drug_pubchemIDs = pd.read_csv(filepath + "drug_PubChem_CIDs.txt", header=None, index_col=0)
    sim_drugs = pd.read_csv(filepath + "drug-drug_similarities_2D.txt", sep=" ", header=None, names=drug_pubchemIDs.index)
    ...
    target_gene_names = pd.read_csv(filepath + "target_gene_names.txt", header=None, index_col=0)
    sim_targets = pd.read_csv(filepath + "target-target_similarities_WS.txt", sep=" ", header=None, names=target_gene_names.index)

    # Load drug-target interaction affinity scores and set the row index with  drug names and column index with gene names
    bindings = pd.read_csv(filepath + "drug-target_interaction_affinities_Kd__Davis_et_al.2011.txt", sep=" ", header=None, names=target_gene_names.index)
...
def eval_model(y_true, y_pred, axis, chart_id):
    ...
    axis[chart_id // 2, chart_id % 2].set_ylim(bottom=2, top=12)
...
def main():
    ...
    kernel1 = GaussianKernel(sim_drugs, gamma=0.01)
    kernel2 = GaussianKernel(sim_targets, gamma=10**-9)
    ...
    kernel1 = GaussianKernel(drugs_train, gamma=0.01)
    kernel2 = GaussianKernel(targets_train, gamma=10**-9)
